...
class IentradaDAO
{
    ...
    public function listarId($id_entrada){
        try{
            $pdo = Conexao::connect();
            $sql = "SELECT id_ientrada,id_produto,qtd,valor_unitario FROM ientrada WHERE id_entrada = :id_entrada";
            ...
            while($linha = $stmt->fetch(PDO::FETCH_ASSOC)){
                $entradas[]=array('id_ientrada'=>$linha['id_ientrada'], 'id_produto'=>$linha['id_produto'], 'qtd'=>$linha['qtd'], 'valor_unitario'=>$linha['valor_unitario']);
                }
        } catch(PDOExeption $e){
